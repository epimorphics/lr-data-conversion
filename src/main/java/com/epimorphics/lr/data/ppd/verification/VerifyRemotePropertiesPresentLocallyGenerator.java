...
public class VerifyRemotePropertiesPresentLocallyGenerator extends PPDVerificationQueryBase implements PPDVerificationQueryGenerator {
	...
	VerifyRemotePropertiesPresentLocallyGenerator(String outputDir, String name, PPDCSVLineFilter lineFilter, String type, String service, String[] filters, ErrorHandler errorHandler) {
	...
	public void generate() {
		...
		sb.append("    {\n");
		sb.append("      SELECT ?transId ?item ?p ?o {\n");
		...
		sb.append("  BIND(CONCAT('missing property value for " + type + " ', STR(?transId), ' (', STR(?p), ', ', STR(?o), ')') AS ?message)\n");
